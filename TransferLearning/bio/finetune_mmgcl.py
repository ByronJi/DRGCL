...
def main():
    ...
    root_supervised = '/pub/data/hujie/zdata/data/BIO/dataset/supervised'
    ...
    with open('result_'+str(args.lr)+'.log', 'a+') as f:
        f.write(
            str(args.runseed) + ' ' + str(np.array(test_acc_easy_list)[np.array(val_acc_list).argmax()]) + ' ' + str(
                np.array(test_acc_hard_list)[np.array(val_acc_list).argmax()]))
        f.write('\n')
