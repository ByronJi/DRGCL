...
class MetaMaskBarlowTwins(nn.Module):
    ...
    def unrolled_backward(self, data, data_aug, model_optim, mask_optim, eta):
        ...
        x_aug, x_aug_cl = self.mask_simclr.forward_cl(data_aug.x, data_aug.edge_index, data_aug.edge_attr, data_aug.batch, masks)
        ...
        x_aug, x_aug_cl = self.mask_simclr_.forward_cl(data_aug.x, data_aug.edge_index, data_aug.edge_attr, data_aug.batch, masks)
        loss = mask_graphcl.simclr_loss(x_cl, x_aug_cl)
...
def main():
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if epoch % 20 == 0:
            torch.save(model.state_dict(), "./models_drgcl/drgcl_" + str(epoch) + ".pth")
